...
class TestTaskManager(unittest.TestCase):
    ...
    def test_generate_report_average_completion_time(self) -> None:
        ...
        total_seconds = (
            timedelta(hours=h1, minutes=m1, seconds=s1)
            + timedelta(hours=h2, minutes=m2, seconds=s2)
        ).total_seconds()
    ...
    def test_complete_task(self) -> None:
        self.storage.get_task.return_value = Task(
            "Task 1", "Description 1", created_at=self.test_date
        )
    ...
    def test_complete_completed_task(self) -> None:
        test_task = Task(
            "Completed Task", "Description 1", True, self.test_date, "1:10:20.0"
        )
